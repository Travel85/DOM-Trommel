...
//get key, call MakeSound and call buttonAnimation
document.addEventListener("keypress", function (event) {
  ...
  try {
    ...
    console.error(`Key ${event.key} not defined!`);
  }
});

//Get click events on array of nodelist, call MakeSound and call buttonAnimation
drums.forEach(function (element) {
  element.addEventListener("click", function () {
    console.log(element);
    let letterClass = element.getAttribute("class").toString().charAt(0);
    console.log(letterClass);
    make_sound(letterClass);
    try {
      buttonAnimation("." + letterClass);
    } catch (error) {
      console.error(`Key ${letterClass} not defined!`);
    }
  });
});
